    ...
    loop {
        ...
        match &mut phase {
            GamePhase::Build => {
                ...
                build.timer -= dt;
                if build.timer <= 0.0 {
                    phase = start_battle(&mut build, &mut units, &mut projectiles, &mut progress);
                    continue;
                }
            ...
            GamePhase::RoundResult { .. } => {
                if is_key_pressed(KeyCode::Space) {
                    if progress.is_game_over() {
                        ...
                        let old_stats: std::collections::HashMap<u64, (f32, f32, f32, f32, u32)> = units
                            .iter()
                            .map(|u| (u.id, (u.damage_dealt_total, u.damage_soaked_total, u.damage_dealt_round, u.damage_soaked_round, u.kills_total)))
                            .collect();

                        // Clear units and respawn all from locked packs
                        units.clear();
                        build = BuildState::new_round(
                            progress.round_gold(),
                            locked_packs,
                            next_id,
                        );
                        ...
                        // Respawn all locked AI units (visible during build phase)
                        units.extend(progress.respawn_ai_units());
        ...
        for unit in &units {
            ...
            draw_circle(unit.pos.x, unit.pos.y, unit.stats.shield_radius, Color::new(tc.r, tc.g, tc.b, alpha));
            draw_circle_lines(unit.pos.x, unit.pos.y, unit.stats.shield_radius, 1.5, Color::new(tc.r, tc.g, tc.b, 0.4 * hp_frac + 0.1));
        }
        ...
        for unit in &units {
            ...
            if unit.slow_timer > 0.0 {
                draw_circle_lines(unit.pos.x, unit.pos.y, unit.stats.size + 3.0, 1.0, Color::new(0.2, 0.5, 1.0, 0.5));
            }
        ...
        for proj in &projectiles {
            ...
            match proj.proj_type {
                ...
                ProjectileType::Rocket => {
                    ...
                    draw_line(tail.x, tail.y, proj.pos.x, proj.pos.y, 3.0, Color::new(1.0, 0.5, 0.2, 0.4));
                    draw_circle(proj.pos.x, proj.pos.y, r, color);
                }
            }
        }
        ...
        match &phase {
            GamePhase::Build => {
                ...
                for (i, placed) in build.placed_packs.iter().enumerate() {
                    ...
                    let thickness = if build.selected_pack == Some(i) { 2.5 } else { 1.5 };
                ...
                // AI pack bounding boxes (from previous rounds, visible during build)
                for ai_pack in &progress.ai_packs {
                    let pack = &packs[ai_pack.pack_index];
                    let half = game_state::PlacedPack::bbox_half_size_rotated(pack, ai_pack.rotated);
                    let min = ai_pack.center - half;
                    let bbox_color = Color::new(0.3, 0.3, 0.5, 0.2);
                    draw_rectangle_lines(min.x, min.y, half.x * 2.0, half.y * 2.0, 1.0, bbox_color);
                    let label = format!("{} (R{})", pack.name, ai_pack.round_placed);
                    draw_text(&label, min.x + 2.0, min.y - 2.0, 12.0, Color::new(0.4, 0.4, 0.6, 0.4));
                }
                ...
                let btn_x = ARENA_W / 2.0 - btn_w / 2.0;
                let btn_y = ARENA_H - 55.0;
                let btn_hovered = mouse.x >= btn_x && mouse.x <= btn_x + btn_w && mouse.y >= btn_y && mouse.y <= btn_y + btn_h;
                let btn_bg = if btn_hovered { Color::new(0.2, 0.6, 0.3, 0.9) } else { Color::new(0.15, 0.4, 0.2, 0.8) };
                ...
                draw_text(btn_text, btn_x + btn_w / 2.0 - tdims.width / 2.0, btn_y + btn_h / 2.0 + 7.0, 22.0, WHITE);

                // Hint text
                draw_text(
                    "Click to select | Double-click to move | Middle-click rotate | Right-click sell",
                    SHOP_W + 10.0, ARENA_H - 10.0, 13.0, Color::new(0.5, 0.5, 0.5, 0.7),
                );
            }
            ...
            GamePhase::Battle => {
                ...
                draw_text(&format!("Red: {}", alive_0), 10.0, ARENA_H - 15.0, 20.0, team_color(0));
                let blue_text = format!("Blue: {}", alive_1);
                let bdims = measure_text(&blue_text, None, 20, 1.0);
                draw_text(&blue_text, ARENA_W - bdims.width - 10.0, ARENA_H - 15.0, 20.0, team_color(1));
            }
            ...
            GamePhase::RoundResult { match_state, lp_damage, loser_team } => {
                ...
                draw_text(&text, ARENA_W / 2.0 - dims.width / 2.0, ARENA_H / 2.0 - 30.0, 36.0, WHITE);

                if let Some(loser) = loser_team {
                    let loser_name = if *loser == 0 { "Player" } else { "AI" };
                    let dmg_text = format!("{} loses {} LP", loser_name, lp_damage);
                    let ddims = measure_text(&dmg_text, None, 22, 1.0);
                    draw_text(&dmg_text, ARENA_W / 2.0 - ddims.width / 2.0, ARENA_H / 2.0 + 5.0, 22.0, Color::new(1.0, 0.4, 0.3, 1.0));
                }
                ...
                draw_text(next_text, ARENA_W / 2.0 - ndims.width / 2.0, ARENA_H / 2.0 + 35.0, 18.0, LIGHTGRAY);
            }

            GamePhase::GameOver(winner) => {
                let text = if *winner == 0 { "YOU WIN!" } else { "YOU LOSE!" };
                let color = if *winner == 0 { Color::new(0.2, 1.0, 0.3, 1.0) } else { Color::new(1.0, 0.3, 0.2, 1.0) };
                let dims = measure_text(text, None, 48, 1.0);
                draw_text(text, ARENA_W / 2.0 - dims.width / 2.0, ARENA_H / 2.0 - 20.0, 48.0, color);

                let round_text = format!("Final Round: {}", progress.round);
                let rdims = measure_text(&round_text, None, 22, 1.0);
                draw_text(&round_text, ARENA_W / 2.0 - rdims.width / 2.0, ARENA_H / 2.0 + 15.0, 22.0, WHITE);

                draw_text("Press R to restart", ARENA_W / 2.0 - 80.0, ARENA_H / 2.0 + 45.0, 18.0, LIGHTGRAY);
            }
        }
...
fn start_battle(
    ...
    // Remove old AI units (they'll be respawned fresh from stored packs)
    units.retain(|u| u.team_id == 0);

    // Respawn all existing AI units from previous rounds at full HP
    units.extend(progress.respawn_ai_units());

    // AI buys techs, then spawns NEW army for this round
    let mut ai_gold = progress.round_gold();
    ai_buy_techs(&mut ai_gold, &mut progress.ai_techs);
    let new_ai_units = progress.spawn_new_ai_army(ai_gold);
    units.extend(new_ai_units);
...
fn draw_hud(progress: &MatchProgress, gold: u32, timer: f32) {
    // Background bar
    draw_rectangle(0.0, 0.0, ARENA_W, 28.0, Color::new(0.05, 0.05, 0.08, 0.85));
    ...
    let plp_color = if progress.player_lp > 500 { Color::new(0.3, 1.0, 0.4, 1.0) }
        else if progress.player_lp > 200 { Color::new(1.0, 0.8, 0.2, 1.0) }
        else { Color::new(1.0, 0.3, 0.2, 1.0) };
    draw_text(&player_lp_text, x, 19.0, 18.0, plp_color);
    x += 160.0;

    // AI LP
    let ai_lp_text = format!("AI LP: {}", progress.ai_lp);
    let alp_color = if progress.ai_lp > 500 { Color::new(0.3, 0.6, 1.0, 1.0) }
        else if progress.ai_lp > 200 { Color::new(1.0, 0.8, 0.2, 1.0) }
        else { Color::new(1.0, 0.3, 0.2, 1.0) };
    draw_text(&ai_lp_text, x, 19.0, 18.0, alp_color);
    x += 140.0;

    // Gold (only during build)
    if gold > 0 || timer > 0.0 {
        let gold_text = format!("Gold: {}", gold);
        draw_text(&gold_text, x, 19.0, 18.0, Color::new(1.0, 0.85, 0.2, 1.0));
        x += 110.0;
...
fn draw_unit_shape(pos: Vec2, size: f32, shape: UnitShape, color: Color) {
    match shape {
        ...
        UnitShape::Square => draw_rectangle(pos.x - size, pos.y - size, size * 2.0, size * 2.0, color),
        UnitShape::Triangle => {
            draw_triangle(vec2(pos.x, pos.y - size), vec2(pos.x - size, pos.y + size), vec2(pos.x + size, pos.y + size), color);
        }
        UnitShape::Diamond => {
        ...
        UnitShape::Star => {
            let s = size;
            draw_triangle(vec2(pos.x, pos.y - s), vec2(pos.x - s * 0.87, pos.y + s * 0.5), vec2(pos.x + s * 0.87, pos.y + s * 0.5), color);
            draw_triangle(vec2(pos.x, pos.y + s), vec2(pos.x - s * 0.87, pos.y - s * 0.5), vec2(pos.x + s * 0.87, pos.y - s * 0.5), color);
        }
        UnitShape::Cross => {
            let arm = size * 0.35;
            ...
            for col in 0..eff_cols {
                if idx < placed.unit_ids.len() {
                    ...
                    if unit.kind == UnitKind::Scout
                        && progress.player_techs.has_tech(UnitKind::Scout, tech::TechId::ScoutEvasion)
                    {
...
fn refresh_units_of_kind(units: &mut [Unit], kind: UnitKind, tech_state: &TechState) {
    for unit in units.iter_mut() {
        ...
        // Apply evasion
        if kind == UnitKind::Scout && tech_state.has_tech(UnitKind::Scout, tech::TechId::ScoutEvasion) {
            unit.evasion_chance = 0.25;
        }
    }
}
