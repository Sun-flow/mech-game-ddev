...
pub fn draw_game_over_ui(
    ...
    let surviving = units.iter().filter(|u| u.team_id == local_pid && u.alive).count();
    let total_units = units.iter().filter(|u| u.team_id == local_pid).count();
